...
public class Swerve extends SubsystemBase {
  ...
  private final SwerveModule m_frontLeft = new SwerveModule(
      Drive.Mod0.DRIVE_MOTOR_ID,
      Drive.Mod0.ANGLE_MOTOR_ID,
      Drive.Mod0.ANGLE_OFFSET);

  private final SwerveModule m_frontRight = new SwerveModule(
      Drive.Mod1.DRIVE_MOTOR_ID,
      Drive.Mod1.ANGLE_MOTOR_ID,
      Drive.Mod1.ANGLE_OFFSET);

  private final SwerveModule m_rearLeft = new SwerveModule(
      Drive.Mod2.DRIVE_MOTOR_ID,
      Drive.Mod2.ANGLE_MOTOR_ID,
      Drive.Mod2.ANGLE_OFFSET);

  private final SwerveModule m_rearRight = new SwerveModule(
      Drive.Mod3.DRIVE_MOTOR_ID,
      Drive.Mod3.ANGLE_MOTOR_ID,
      Drive.Mod3.ANGLE_OFFSET);
  ...
  public void drive(double xSpeed, double ySpeed, double rot, boolean fieldRelative, boolean rateLimit) {
    ...
    if (rateLimit) {
      ...
      double angleDif = Util.AngleDifference(inputTranslationDir, m_currentTranslationDir);
      if (angleDif < 0.45*Math.PI) {
        m_currentTranslationDir = Util.StepTowardsCircular(m_currentTranslationDir, inputTranslationDir, directionSlewRate * elapsedTime);
        ...
        if (m_currentTranslationMag > 1e-4) { //some small number to avoid floating-point errors with equality checking
          ...
          m_currentTranslationDir = Util.WrapAngle(m_currentTranslationDir + Math.PI);
          m_currentTranslationMag = m_magLimiter.calculate(inputTranslationMag);
        }
      }
      else {
  ...
  public double getTurnRate() {
    return m_gyro.getRate(IMUAxis.kZ) * (Drive.kGyroReversed ? -1.0 : 1.0);
  }
}
